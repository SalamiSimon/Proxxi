...
class ResponseModifier:
    ...
    def _url_matches(self, flow_url: str, target_url: str) -> bool:
        ...
        if target_url.startswith('^') or any(c in target_url for c in '.*+?[](){}|'):
            try:
                ...
                print(f"[DEBUG] Falling back to substring match: {result}")
                return result
                
        # Simple substring match
        result = target_url in flow_url
        print(f"[DEBUG] URL substring match: {flow_url} contains {target_url} => {result}")
        return result
    ...
    def _find_matching_targets(self, flow: http.HTTPFlow) -> List[Dict[str, Any]]:
        ...
        for target in self.targets:
            ...
            print(f"[DEBUG] Checking target ID={target['id']}: URL match={url_match}, status match={status_match}")
